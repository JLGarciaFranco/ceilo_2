...
#	plt.xlim([0,24])
	#if wstd:
	#	plt.fill_between(tvec,y-yerr,y+yerr,color='wheat')
	if save:
		plt.savefig(name)
	if show:
		plt.show()
...
def longmaxmin(meses, maximo):
	...
	while startyear!=2017:
		...
		print startyear, mesesits[index[0]], mesesits[indicit[0]]
		print value-valmin 
		startyear+=1
...
def pullclouds(df,cloudfile):
	...
	print len(cloudf),len(df)
	print float(len(cloudf))/float(len(df))
	dfsnubs=df.copy()
...
#		print m1
		...
		print datetime.datetime(date[0].year,date[0].month,1)
	#	plt.plot(date,x)
	...
	print len(meses),len(meanly),len(dates),len(dt)
