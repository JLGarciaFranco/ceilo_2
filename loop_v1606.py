...
for j in lista:
	carpeta=directorio
	print carpeta
	filelist=glob.glob(carpeta+"*7*.DAT")
	print carpeta
	filelist=filelist+glob.glob(carpeta+"*8*.DAT")
	...
	for i,filename in enumerate(filelist):
		...
		if oldfile[0:6]!=files[0:6]:
			...
			comandline=  ('python ceilov201605.py %s %s %s %s %s %s %s' % (estacion, carpeta,fdirectory,runv,dt,writeonly,files) )
			j=1
